...
function calculate() {
  ...
  if(equation === '') result = 0;
    ...
    if( isOperator(equation[len - 1]) ) 
      equation = equation.slice(0, len - 1);
...
function calculatePrecedence({ operands, operators }){
  ...
  function resultHelper(operands, operators, index){
    let replacementResult = calculateOperands(operands[index], operands[index+1], operators[index]);
  ...
  while(operators.length !== 0){  
    ...
    else if(i === operators.length){ // the i bumped into the wall not finding * or /
      while(operators.length !== 0)
        resultHelper(operands, operators, 0);
    }
    else 
      i++;
  }
...
function calculateOperands(op1, op2, operator){
  switch(operator){
    case '+': return parseInt(op1) + parseInt(op2);
    case '-': return op1 - op2;
    case '*': return op1 * op2;
    case '/': return op1 / op2;
    default: return -1;
